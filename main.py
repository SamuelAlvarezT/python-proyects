...
client = TelegramClient('35Intento', api_id, api_hash)
...
async def nuevo_mensaje(event):
    ...
    print( f" {Fore.WHITE} Nuevo Mensaje recibido")
    ...
    # Si el mensaje pasa los filtros, añádelo a la cola y al set
     # Si el mensaje pasa los filtros, añádelo a la cola y al set, y guárdalo en el archivo JSON
    cola_de_mensajes.put(mensaje)
    mensajes_set.add(mensaje)
    mensajes_filtrados.append(mensaje)  # Agrega el mensaje a la lista de mensajes filtrados
    with open(json_file_path, 'w') as file:
        json.dump(list(mensajes_filtrados), file)  #Guarda lista mensajes filtrados en el JSON
